let ignore = ["/wp-","#respond", "javascript:"];

$(document).on("click", "a[target!=_blank]", function (e) {
    let href = $(this).attr("href");
    if (!ajax_check(href)) return false;
    ajax(href, "pagelink");
    return false;
});
...
function ajax_check(url) {
    if (!url) return false;
    for (let i of ignore) {
        if (url.indexOf(i) >= 0) return false;
    }
    return true;
}
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            window.history.pushState({
                ...
                html: $(data).filter("#page").html()
            },$(data).filter("title").text(),reqUrl);
            $("#page").html($(data).filter("#page").html());
            if (msg === "pagelink") {
                let anchor = window.location.hash.substring(location.hash.indexOf("#") + 1);
                if (anchor) {
                    $("html").animate({
